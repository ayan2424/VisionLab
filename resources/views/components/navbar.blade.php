<header class="sticky top-0 z-30 flex items-center justify-between h-16 px-6 transition-colors duration-300 border-b backdrop-blur-md"
    ...
    <div class="flex items-center gap-4">
        ...
        <button @click="mobileMenuOpen = !mobileMenuOpen" aria-label="Open mobile menu" class="md:hidden p-2 rounded-lg transition-colors" style="color:var(--vc-text-secondary);">
    ...
    <div class="flex items-center gap-3">
        ...
        <!-- Theme Toggle Button -->
        <button onclick="window.themeManager.toggle()" aria-label="Toggle Theme" class="p-2 rounded-full transition-colors hover:bg-vc-elevated" style="color:var(--vc-text-secondary);" title="Toggle Theme">
        ...
        <div x-data="notifBell()" class="relative">
            <button @click="toggle()" aria-label="Notifications" class="p-2 rounded-full transition-colors relative hover:bg-vc-elevated" style="color:var(--vc-text-secondary);">
        ...
        <form method="POST" action="{{ route('logout') }}" class="ml-2">
            @csrf
            <button type="submit" aria-label="Sign Out" class="p-2 rounded-full transition-colors hover:bg-red-500/10 text-red-500" title="Sign Out">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
