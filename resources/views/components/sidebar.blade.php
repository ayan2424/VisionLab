<aside id="dash-sidebar" class="vc-card fixed inset-y-0 left-0 z-50 w-64 md:relative md:w-[280px] transition-all duration-400 transform -translate-x-full md:translate-x-0 flex flex-col flex-shrink-0 !rounded-2xl overflow-hidden">
    ...
    <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-1.5">
        ...
        <div class="pt-6 mt-6 border-t" style="border-color:rgba(255,255,255,0.05);">
            ...
            @php $gActive = request()->routeIs('profile.index'); @endphp
            <a href="{{ route('profile.index') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium transition-all duration-200 group relative overflow-hidden"
               style="{{ $gActive ? 'color:#EAB308;background:rgba(234,179,8,0.08);font-weight:600;' : 'color:var(--vc-text-secondary);' }}">
                @if($gActive)<span class="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full" style="background:#EAB308;"></span>@endif
                <svg class="w-[18px] h-[18px]" style="{{ $gActive ? 'color:#EAB308;' : 'color:var(--vc-muted);' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
