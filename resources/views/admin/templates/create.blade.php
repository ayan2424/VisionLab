...
<div class="vc-card max-w-4xl">
    <form method="POST" action="{{ route('admin.templates.store') }}">
        ...
        <div class="mb-6">
            ...
            <textarea name="nix_config" rows="10" class="w-full rounded-md px-3 py-2 text-sm border focus:ring-1 focus:outline-none transition-colors font-mono" style="background:#0a0a0a;border-color:var(--vc-border);color:#e5e7eb;" placeholder="# Write Nix expression here...&#10;{ pkgs ? import <nixpkgs> {} }:&#10;pkgs.mkShell {&#10;  buildInputs = [&#10;    pkgs.php83&#10;    pkgs.php83Packages.composer&#10;  ];&#10;}">{{ old('nix_config') }}</textarea>
        ...
        <div class="mb-6">
            ...
            <textarea name="ui_parameters" rows="4" class="w-full rounded-md px-3 py-2 text-sm border focus:ring-1 focus:outline-none transition-colors font-mono" style="background:#0a0a0a;border-color:var(--vc-border);color:#e5e7eb;" placeholder='[{"name": "framework", "label": "Framework Version", "type": "select", "options": ["Laravel 10", "Laravel 11"]}]'>{{ old('ui_parameters') }}</textarea>
