...
<div class="max-w-3xl mx-auto">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white mb-1">Add Department</h1>
            <p class="text-slate-400 text-sm">Register a new academic department.</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.departments.store') }}" class="vc-card p-6 border-t-4" style="border-top-color:var(--vc-accent);">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required class="vc-input w-full" placeholder="e.g. Computer Science">
                @error('name') <span class="text-xs text-red-500 mt-1">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="block text-sm font-bold mb-2 text-slate-300">Code</label>
                <input type="text" name="code" value="{{ old('code') }}" class="vc-input w-full" placeholder="e.g. CS">
        ...
        <div class="mb-6">
            ...
            <textarea name="description" rows="3" class="vc-input w-full">{{ old('description') }}</textarea>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="btn-primary px-6 py-2.5 rounded-xl text-sm font-semibold shadow-lg">Save Department</button>
        </div>
    </form>
</div>
@endsection
