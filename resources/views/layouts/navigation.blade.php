...
<nav x-data="{ open: false }" class="sticky top-0 z-50 border-b transition-colors duration-300"
            ...
            <div class="flex items-center gap-6">
                ...
                <div class="hidden sm:flex items-center gap-1">
                    ...
                    <a href="{{ route('gamification.index') }}"
                       class="px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 flex items-center gap-1"
                       style="{{ request()->routeIs('gamification.*')
    ...
    <div :class="{'block': open, 'hidden': !open}" class="hidden sm:hidden" style="border-top:1px solid var(--vc-border);">
        <div class="px-4 py-3 space-y-1">
            ...
            @if(Auth::check() && Auth::user()->isStudent())
            <a href="{{ route('gamification.index') }}" class="block px-3 py-2 rounded-lg text-sm transition-all" style="color:#EAB308;">Rank / Gamification</a>
            @endif
            @if(Auth::check() && Auth::user()->isAdmin())
