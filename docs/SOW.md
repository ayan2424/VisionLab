...
| 9.0 | Analytics, Forensics, Gamification, Deployment | Analytics event taxonomy (20+ types via Model Observers), 7 Chart.js admin dashboards, VisionGuard forensics (keystroke attribution, confidence level), 365-day contribution heatmap, daily streak tracking, 10 achievement badges, student deployment (Vercel/Railway provider abstraction, package exclusions, real-time status) |
| 10.0 | PWA and Notifications | Web app manifest (app shortcuts, screenshots), Workbox 7 service worker (5 route strategies, cache versioning), offline fallback page, install prompt + update banner, VAPID push (3 notification classes, scheduled reminders), Background Sync for offline submissions |
...
| D-10 | Analytics and Forensics | Event taxonomy, dashboards, VisionGuard, heatmap, streaks, badges | Role-restricted analytics tests pass; VisionGuard forensics display confirmed |
...
| M-09 | Analytics and Deployment Accepted | 9 | Role-restricted analytics confirmed, VisionGuard display correct, deployment lifecycle passes |
