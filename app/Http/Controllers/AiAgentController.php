...
class AiAgentController extends Controller
{
    ...
    //  Gemini HTTP helpers
    // ─────────────────────────────────────────────────────────────
    private function callGemini(string $system, string $user, string $mode, string $code): JsonResponse
    {
        try {
    ...
    private function parseGeminiResponse(string $text, string $mode, string $originalCode): JsonResponse
    {
        $snippet = null;
        if (preg_match('/```(?:\w+)?\n(.*?)```/s', $text, $m)) {
            $snippet = trim($m[1]);
    ...
    private function callGeminiPatch(string $prompt, string $original, string $filename): JsonResponse
    {
        try {
            $url = "{$this->geminiEndpoint}/{$this->geminiModel}:generateContent?key={$this->geminiKey}";
            $response = Http::timeout(30)->post($url, [
    ...
    // ─────────────────────────────────────────────────────────────
    private function demoResponse(string $message, string $mode, string $language, string $code, string $filename): JsonResponse
    {
        ...
        [$reply, $snippet] = match (true) {
            str_contains($msg, 'explain') || str_contains($msg, 'what does') || str_contains($msg, 'how does') => [$this->explainCode($code, $language, $lines, $filename), null],

            str_contains($msg, 'bug') || str_contains($msg, 'error') || str_contains($msg, 'fix') || str_contains($msg, 'wrong') => [$this->bugHints($language), $this->bugFixSnippet($language)],

            str_contains($msg, 'optim') || str_contains($msg, 'faster') || str_contains($msg, 'performance') || str_contains($msg, 'slow') => [$this->optimizeResponse($language, $lines), null],

            str_contains($msg, 'test') || str_contains($msg, 'spec') || str_contains($msg, 'unit') => ["Here's a starter test suite for your <strong>{$language}</strong> code:", $this->testSnippet($language)],

            str_contains($msg, 'complex') || str_contains($msg, 'big o') || str_contains($msg, 'o(n') => [$this->complexityAnalysis(), null],

            str_contains($msg, 'artifact') || str_contains($msg, 'component') || str_contains($msg, 'design') || str_contains($msg, 'ui') => ["Here is the requested component:\n\n<vision_artifact title=\"Login Form\" language=\"html\">\n<div class=\"p-8 bg-slate-900 rounded-2xl shadow-2xl border border-slate-800 max-w-sm mx-auto mt-10\">\n  <h2 class=\"text-2xl font-bold text-white mb-6 text-center\">Welcome Back</h2>\n  <form>\n    <div class=\"mb-4\">\n      <label class=\"block text-xs font-bold text-slate-400 mb-2\">EMAIL</label>\n      <input type=\"email\" class=\"w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-3 text-slate-200 focus:outline-none focus:border-violet-500 transition-colors\" placeholder=\"you@example.com\">\n    </div>\n    <div class=\"mb-6\">\n      <label class=\"block text-xs font-bold text-slate-400 mb-2\">PASSWORD</label>\n      <input type=\"password\" class=\"w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-3 text-slate-200 focus:outline-none focus:border-violet-500 transition-colors\" placeholder=\"••••••••\">\n    </div>\n    <button type=\"button\" class=\"w-full bg-violet-600 hover:bg-violet-700 text-white font-bold py-3 px-4 rounded-lg transition-colors\">\n      Sign In\n    </button>\n  </form>\n</div>\n</vision_artifact>", null],

            str_contains($msg, 'refactor') || str_contains($msg, 'clean') || str_contains($msg, 'improve') => ["Here are <strong>3 refactoring opportunities</strong> in <strong>{$filename}</strong>:\n\n".$this->refactorSuggestions(), null],

            str_contains($msg, 'hello') || str_contains($msg, 'hi') || str_contains($msg, 'hey') => ["Hello! I'm the VisionLabAgent. I can help you explain code, find bugs, optimize performance, write tests, and apply patches. What would you like to do with <strong>{$filename}</strong>?", null],
    ...
    {
        if (! $code) {
            return $this->helloWorld($language);
        }
    ...
    private function demoPatch(string $instruction, string $original, string $filename, string $language): JsonResponse
    {
        return response()->json([
