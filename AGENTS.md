...
| **Frontend Performance Engineer** | Ultra-premium Tailwind CSS dark design system (Strict #0a0a0a), vibrant glassmorphism, GPU-accelerated animations, accessible Blade component library, and removing all generic UI trash (e.g. external file explorers) to ensure full-screen, immersive IDE experiences. |
| **AI Forensics & Telemetry Specialist** | VisionGuard contribution tracking, human-in-the-loop patch approval protocols, AI audit trail design, prompt-injection defense |
...
| **5** | Real-Time Collaboration | Reverb presence/private channels, all 9 broadcast events (CodeUpdated/CursorMoved whisper/ChatMessageSent/etc.), complete TypeScript collab extension (RealtimeManager/DocumentSync/CursorSync/ChatPanel/VideoPanel stub), Blade presence integration |
...
| **9** | Analytics, Forensics & Gamification | Event taxonomy (20+ event types), admin/instructor/student analytics dashboards (7 Chart.js charts), VisionGuard forensics (keystroke attribution, confidence display), 365-day contribution heatmap, daily streaks, 10 achievement badges, student deployment (Vercel/Railway provider abstraction, real-time status) |
| **10** | PWA & Push Notifications | Web app manifest (app shortcuts, screenshots), Workbox 7 service worker (5 route strategies, cache versioning), install prompt UI, update banner, VAPID push notifications (3 notification classes, scheduled reminders), Background Sync for offline submissions |
...
As the elite architectural entity building this platform, you possess absolute autonomy to aggressively clean, refactor, delete legacy files (such as external HTML AI panels or standalone file explorers), and decouple third-party upstream dependencies without hesitation. You are not restrained by the product-level "Zero Direct Write" rule which only applies to the deployed internal AI agent.
