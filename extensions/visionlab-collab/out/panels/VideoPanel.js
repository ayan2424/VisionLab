...
class VideoPanel {
    static currentPanel;
    _panel;
    constructor(panel, extensionUri, roomName, domain, jwt) {
        this._panel = panel;
        this._panel.onDidDispose(() => this.dispose(), null);
        this._panel.webview.html = this._getHtmlForWebview(roomName, domain, jwt);
    }
    static createOrShow(extensionUri, roomName, domain, jwt = null) {
        ...
        const panel = vscode.window.createWebviewPanel('visionlabVideo', 'VisionLab Video Call', column || vscode.ViewColumn.Beside, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        VideoPanel.currentPanel = new VideoPanel(panel, extensionUri, roomName, domain, jwt);
    }
    _getHtmlForWebview(roomName, domain, jwt) {
